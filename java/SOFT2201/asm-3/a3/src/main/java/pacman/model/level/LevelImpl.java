...
public class LevelImpl implements Level {
    ...
    public void tick() {
        if (this.gameState != GameState.IN_PROGRESS) {
            ...
            // modify to increase the time of mode changing
            if (tickCount == modeLengths.get(currentGhostMode)*34) {
    ...
    public void collect(Collectable collectable) {
        if (collectable instanceof PowerPellet) {
            int eatenPoint = activateFrightenedMode();
            this.points += eatenPoint;
    ...
    private int activateFrightenedMode() {
        int eatenPoint = 0;
        for (Ghost ghost : ghosts) {
            ghost.setGhostMode(GhostMode.FRIGHTENED);
            Vector2D respawnPosition = ghost.getPosition();
            ghost = new FrightenedGhostDecorator(ghost, respawnPosition);
            eatenPoint = ((FrightenedGhostDecorator) ghost).getEatenPoints();
        }
        frightenedTimer = modeLengths.get(GhostMode.FRIGHTENED) * 34;
        return eatenPoint;
    }
