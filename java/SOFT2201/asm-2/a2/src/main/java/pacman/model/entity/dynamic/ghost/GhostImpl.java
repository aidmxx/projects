...
public class GhostImpl implements Ghost {
    ...
    private Vector2D getTargetLocation() {
        return switch (this.ghostMode) {
            ...
            case CHASE -> {
                // playerPosition could be null under SCATTER mode
                if (this.playerPosition == null) {
                    throw new IllegalStateException("player position not set");
                }
                // pacman position in chase mode
                // yield used to return values from a switch expression
                yield this.playerPosition;
            }
    ...
    public void setGhostMode(GhostMode ghostMode) {
        if (speeds == null || speeds.get(ghostMode) == null) {
            throw new IllegalStateException("Speeds map is not properly initialized for GhostMode: " + ghostMode);
        }
        this.ghostMode = ghostMode;
    ...
    public void reset() {
        ...
        this.kinematicState = new KinematicStateImpl.KinematicStateBuilder()
                ...
                .build();
        // back to initial SCATTER mode after reset
    ...
    // used to update pacMan current position to pass to ghosts
