...
EXPECTED_OUTPUT="merk_test2.out"          # The name of the expected output file
...
nhashes=$(grep "nhashes:" "$CURRENT_DIR/$INPUT" | awk -F: '{print $2}' | xargs)
echo "Number of hashes: $nhashes"

# Check if nhashes is a valid number
if [[ ! "$nhashes" =~ ^[0-9]+$ ]]; then
    echo "Error: nhashes is not a valid number"
    exit 1
fi

# Keep only the lines corresponding to nhashes from the top of the actual output
head -n "$nhashes" "$CURRENT_DIR/$ACTUAL_OUTPUT" > "$CURRENT_DIR/$TEMP_OUTPUT"
